...
const int fireDist = 25;   // threshold for guard mode firing, in centimeters
...
void reload()
{
  ...
  loader.write(165);  // still chambering another dart
  if (checkAuto)
  {
    digitalWrite(13, LOW);    // blink blue LED to show you're in automatic mode
  }
  delay(5000);  // delay to wait for pullback
  ...
  analogWrite(Motor2Speed, 130);
  loader.write(10);  // still dropping the dart
  trigger.write(80);   // still setting the trigger up
  if (checkAuto)
  {
    digitalWrite(13, HIGH);    // blink blue LED to show you're in automatic mode
  }
  delay(3000);  // delay to wait for draw motors to unwind
  ...
  digitalWrite(12, HIGH);    // turn the red LED on by making the voltage HIGH
